...
export default function AdminResources() {
  ...
  const confirmDelete = (id) => { setDeletingId(id); setShowConfirm(true) }
  ...
  const toggleStatus = async (r) => {
    ...
    try { await resourceService.update(r.id, { status: next }); load() }
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">Create, edit and remove campus resources</div>
        </div>
      </div>

      <div style={{ marginBottom: 16 }}>
        {user?.role === 'ADMIN' && <button className="btn btn-primary" onClick={openCreate}>Add Resource</button>}
      </div>
      ...
      {loading ? (
        ...
        <div className="table-wrapper">
          <table className="table">
            <thead>
              <tr><th>Name</th><th>Type</th><th>Location</th><th>Capacity</th><th>Availability</th><th>Status</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {resources.map(r => (
                <tr key={r.id}>
                  ...
                  <td><span className={`status-badge ${r.status === 'ACTIVE' ? 'approved' : 'cancelled'}`}>{r.status}</span></td>
                  <td>
                    <div style={{ display: 'flex', gap: 8 }}>
                      {user?.role === 'ADMIN' && (
                        <>
                          <button className="btn btn-outline btn-sm" onClick={() => openEdit(r)}>Edit</button>
                          <button className="btn btn-danger btn-sm" onClick={() => confirmDelete(r.id)}>Delete</button>
                          <button className="btn btn-ghost btn-sm" onClick={() => toggleStatus(r)}>{r.status === 'ACTIVE' ? 'Mark OUT_OF_SERVICE' : 'Mark ACTIVE'}</button>
                        </>
                      )}
                      {user?.role === 'OPERATIONS' && (
                        <div style={{ fontSize: 13, color: 'var(--gray-600)' }}>Read-only</div>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ width: 720, background: 'white', borderRadius: 8, padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontWeight: 700 }}>{editing ? 'Edit Resource' : 'Create Resource'}</div>
      ...
      {showConfirm && (
        <ConfirmDelete title="Delete resource?" description="Deleting a resource is irreversible." onCancel={() => setShowConfirm(false)} onConfirm={handleDelete} loading={false} />
      )}
    </div>
  )
}
