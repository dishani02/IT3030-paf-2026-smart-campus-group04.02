...
import { 
    Building2, MapPin, Users, Clock, Wifi, 
    ArrowLeft, Calendar, Wrench, Edit, Trash2,
    CheckCircle, AlertCircle, Info, FlaskConical, Zap
} from 'lucide-react'
...
export default function ResourceDetail() {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (err.response?.status === 409) {
                alert('Conflict: This asset cannot be removed while active bookings exist.')
            }
        } finally {
            setDeleting(false)
    ...
    if (!resource) return (
        <div className="max-w-7xl mx-auto p-12 text-center">
            ...
            <h2 className="text-xl font-bold text-slate-800 mb-2">Asset Not Found</h2>
            <p className="text-slate-400 text-sm mb-6">The campus resource you are looking for does not exist or has been retired.</p>
            <button onClick={() => navigate('/resources')} className="btn btn-primary">Return to Catalog</button>
        </div>
    )

    const typeStyle = TYPE_CONFIG[resource.type?.toUpperCase()] || TYPE_CONFIG.DEFAULT
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-8 space-y-8">
                    ...
                    {resource.images && resource.images.length > 0 ? (
                        <div 
                            ...
                            onClick={() => { setGalleryImages(resource.images); setShowGallery(true); }}
                        >
                            <img src={`http://localhost:8080${resource.images[0]}`} alt={resource.name} className="w-full h-full object-cover transition-transform duration-500 group-hover/banner:scale-105" />
                            ...
                            <div className="absolute bottom-6 left-8 flex items-end gap-6 z-10">
                                ...
                                <div className="text-left mb-1">
                                     <p className="text-white font-black text-2xl tracking-tight leading-none drop-shadow-md">{resource.name}</p>
                                     <p className="text-white/80 text-[11px] font-bold uppercase tracking-widest mt-2">{typeStyle.label} Unit #{resource.id}</p>
                                </div>
                            </div>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/banner:opacity-100 bg-black/20 transition-opacity z-20">
                                <span className="text-white text-[12px] font-bold tracking-wider uppercase backdrop-blur-md px-5 py-2.5 rounded-xl bg-black/40 border border-white/10 shadow-xl">View Gallery</span>
                            </div>
                        </div>
                    ) : (
                        <div className={`relative p-8 rounded-3xl overflow-hidden border-2 border-white shadow-sm flex items-center gap-6 ${typeStyle.bannerBg}`}>
                            ...
                            <div className={`p-4 bg-white rounded-2xl shadow-lg border ${typeStyle.border} relative z-10`}>
                                <Icon className={`w-10 h-10 stroke-[1.5px] ${typeStyle.text}`} />
                            </div>
                            <div className="relative z-10 text-left">
                                 <p className="text-slate-800 font-black text-2xl tracking-tight leading-none">{resource.name}</p>
                                 <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest mt-2">{typeStyle.label} Unit #{resource.id}</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden p-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-6 border-y border-slate-50">
                            <div className="space-y-1">
                                <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Max Occupancy</p>
                                <div className="flex items-center gap-2 text-slate-700 font-bold">
                                    <Users className="w-4 h-4 text-blue-500" /> {resource.capacity || 'Flexible'} Seats
                                </div>
                            </div>
                            <div className="space-y-1 text-left">
                                <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Availability</p>
                                <div className="flex items-center gap-2 text-slate-700 font-bold">
                                    <Clock className="w-4 h-4 text-blue-500" /> {resource.availabilityStart} – {resource.availabilityEnd}
                                </div>
                            </div>
                            <div className="space-y-1 text-right md:text-left">
                                <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Connectivity</p>
                                <div className="flex items-center gap-2 text-slate-700 font-bold">
                        ...
                        <div className="mt-8 space-y-4">
                            <p className="text-[11px] font-bold text-slate-300 uppercase tracking-widest leading-none">Description</p>
                            <p className="text-[14px] text-slate-500 leading-relaxed font-medium">
                                {resource.description || 'No formal description provided for this campus asset.'}
                            </p>
                        </div>
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 text-center space-y-4">
                        ...
                        <div>
                            <h4 className="font-bold text-slate-800 text-[14px]">Report Malfunction</h4>
                            <p className="text-[12px] text-slate-400 mt-1">If this resource is damaged or requires maintenance, please alert facilities.</p>
                        </div>
                        <button 
                            onClick={() => navigate('/tickets', { state: { resourceId: resource.id } })}
                    ...
                    <div className="bg-blue-50/50 rounded-3xl p-7 border border-blue-100">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <h5 className="font-bold text-slate-800 text-[14px]">Guidelines</h5>
                        </div>
                    ...
                    <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                        <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest mb-6">Load Factor</p>
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-100 animate-in fade-in zoom-in duration-200">
                        <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
                            ...
                            <button onClick={() => setShowEditForm(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-50 text-slate-400 transition-colors">&times;</button>
                        </div>
                        <div className="p-8">
                            <ResourceForm
                                resource={resource}
                                onSaved={() => { setShowEditForm(false); loadResource() }}
