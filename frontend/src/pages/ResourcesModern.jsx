...
import {
    MapPin, Users, Search, Plus, Edit,
    Trash2, Eye, Calendar, Building2, FlaskConical, Wrench
} from 'lucide-react'
...
const TYPE_CONFIG = {
    ROOM: { icon: Building2, label: 'Room', bg: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-100' },
    LAB: { icon: FlaskConical, label: 'Lab', bg: 'bg-violet-50', text: 'text-violet-600', border: 'border-violet-100' },
    EQUIPMENT: { icon: Wrench, label: 'Equipment', bg: 'bg-amber-50', text: 'text-amber-600', border: 'border-amber-100' },
}
...
const TYPE_IMAGES = {
    ...
    EQUIPMENT: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=600&h=400&fit=crop&q=80',
}
...
export default function ResourcesModern() {
    ...
    const handleBook = (resource) => navigate('/bookings', { state: { resourceId: resource.id } })
    ...
    const toggleStatus = async (resource) => {
        ...
        await resourceService.update(resource.id, { ...resource, status: newStatus })
        loadResources()
    }
    ...
    return (
        <div className="space-y-8">
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-100/50">
                <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-0.5">
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight leading-tight">Facilities & Assets</h1>
                        <p className="text-sm text-slate-500 font-medium">Discover and reserve academic spaces for research and collaboration.</p>
                    </div>
                </div>
                {isAdmin && (
                    <button
            ...
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <span className="text-[13px] text-slate-500 font-medium">{activeCount} Available Assets</span>
                <div className="flex items-center gap-3">
                    <span className="text-[11px] text-slate-400 font-semibold uppercase tracking-wider">Sort:</span>
                    <select
                        value={sortBy}
            ...
            {filtered.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 p-16 text-center">
                    ...
                    <h3 className="text-[15px] font-semibold text-slate-800 mb-1">No resources found</h3>
                        ...
                        return (
                            ...
                            >
                                ...
                                {resource.images && resource.images.length > 0 && (
                                    ...
                                    >
                                        <img src={`http://localhost:8080${resource.images[0]}`} alt={resource.name} className="w-full h-full object-cover transition-transform duration-500 group-hover/img:scale-105" />
                                ...
                                <div className={`${config.bg} px-4 py-3 flex items-center justify-between`}>
                                    <div className="flex items-center gap-2">
                                        <div className={`w-7 h-7 bg-white rounded-lg flex items-center justify-center ${config.border}`}>
                                            <TypeIcon className={`w-3.5 h-3.5 ${config.text}`} />
                                        </div>
                                        <span className={`${config.text} text-[10px] font-bold uppercase tracking-widest`}>{config.label}</span>
                                    </div>
                                ...
                                <div className="px-4 py-3">
                                    ...
                                    {resource.description && (
                                        <p className="text-[11px] text-slate-400 line-clamp-1 mb-2">{resource.description}</p>
                                    )}
                ...
                <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-100">
                        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                            ...
                            <button onClick={() => setShowForm(false)} className="text-slate-400 hover:text-slate-600 transition-colors text-xl leading-none">&times;</button>
                        </div>
                        <div className="p-6">
                            <ResourceForm
                                resource={editing}
